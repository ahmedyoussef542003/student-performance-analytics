...
class ProfessionalSchoolApp(ctk.CTk):
    ...
    def get_or_create_student(
        ...
        cursor.execute(
            "SELECT student_id, section FROM Students WHERE student_name = ?",
            (name,),
        )
        row = cursor.fetchone()
        if row:
        ...
        cursor.execute(
            "INSERT INTO Students (student_name, grade_level, class_name,"
            " section) VALUES (?, ?, ?, ?)",
    ...
    def setup_views(self):
        ...
        # 4. Student Report View (تنسيق الصفحة باستخدام المكون المدمج)
    ...
    def save_skill(self):
        ...
        for sk_name, is_m in valid_entries:
            cursor.execute(
                "INSERT INTO Skills (student_id, subject, skill_name,"
                " is_mastered) VALUES (?, ?, ?, ?)",
